...
namespace ns3 {
		...
		QbbNetDevice::GetTypeId(void)
	{
		static TypeId tid = TypeId("ns3::QbbNetDevice")
			...
			.AddAttribute("QcnEnabled",
				...
				MakeBooleanChecker())
			.AddAttribute("FcmEnabled",
				"Enable the generation of fcm packet.",
				BooleanValue(false),
				MakeBooleanAccessor(&QbbNetDevice::m_fcmEnabled),
				MakeBooleanChecker())
	...
	QbbNetDevice::QbbNetDevice()
	{
		...
		/*fcm initial*/
		gradeFcm = m_bps.GetBitRate()/2560000;
		for(int i; i<qCnt; ++i){
			m_rate[i] = m_bps;
			m_credits[i] = 0;
		...
		QbbNetDevice::DequeueAndTransmit(void)
	{
		...
		if (m_node->GetNodeType() == 0){
			...
			if(m_fcmEnabled){
				p = m_queue->DequeueFcm(m_nextAvail);  //fcm modification, use to throttle sending rate
			}else{
			...
			if (p != 0){
				...
				if(m_fcmEnabled){                       //use qcn method to limit the rate of FCM
					//if (m_rate[qIndex] == 0)			//late initialization	
					//{
					//	m_rate[qIndex] = m_bps;		
					//}
					double creditsDue = std::max(0.0, m_bps / m_rate[qIndex] * (p->GetSize() - m_credits[qIndex]));
					Time nextSend = m_tInterframeGap + Seconds(m_bps.CalculateTxTime(creditsDue));
					m_nextAvail[qIndex] = Simulator::Now() + nextSend;
					for (uint32_t i = 0; i < qCnt; i++)	//distribute credits
					{
						if (m_nextAvail[i].GetTimeStep() <= Simulator::Now().GetTimeStep())
							m_credits[i] += m_rate[i] / m_bps*creditsDue;
					}
					m_credits[qIndex] = 0;	//reset credits
				}
				...
				NS_LOG_INFO("PAUSE or fcm prohibits send at node " << m_node->GetId());
			...
			if (m_node->GetNodeType() > 0){ // switch
				...
				if(m_fcmEnabled){  
					for(int i = 0; i < qCnt; ++i){
						if(ch.udp.ih.qlenFcm[i]!=0){
							m_rate[i] = m_bps - ch.udp.ih.qlenFcm[i]*gradeFcm;							
						}
					}
				}
				if(ch.dip == Ipv4Address("255.255.255.255").Get())return; //if this packet is generated by switch, drop after obtaining the qlen
